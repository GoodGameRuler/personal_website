...
vec3 base=vec3(.996,.978,.952);
vec3 cloud=vec3(.972,.912,.885);
vec3 rose=vec3(.93,.45,.62);
vec3 berry=vec3(.86,.55,.66);
vec3 col=mix(base,cloud,smoothstep(.35,.85,cl)*.55);
vec2 d1=vec2(t*1.6,-t*1.1);
float p1=noise(vec2(p.x*6.5,p.y*10.)+d1*3.+r*2.);
float pet1=smoothstep(.86,.94,p1);
vec2 d2=vec2(-t*1.1,-t*1.7);
float p2=noise(vec2(p.x*10.,p.y*15.)+d2*3.+q*2.);
float pet2=smoothstep(.89,.96,p2);
col=mix(col,rose,pet1*.8);
col=mix(col,berry,pet2*.55);
col*=.96+.05*uv.y;
