...
s1=pow(smoothstep(.64,1.,s1),3.);
float s2=1.-abs(2.*fbm(p*1.6+2.2*q-t*.6+vec2(3.1,7.7))-1.);
s2=pow(smoothstep(.70,1.,s2),3.);
vec3 base=vec3(.995,.972,.940);
vec3 mid=vec3(.985,.925,.885);
vec3 rose=vec3(.93,.38,.58);
vec3 crimson=vec3(.84,.45,.58);
vec3 gold=vec3(.92,.64,.28);
vec3 col=mix(base,mid,f);
col=mix(col,rose,s1*.6);
col=mix(col,crimson,s2*.45);
